...
_.extend(VMClient.prototype, {
  ...
  getDeployments: function (options, callback) {
    var computeManagementClient = this.createComputeManagementClient();
    progress = self.cli.interaction.progress($('Getting virtual machines'));
    getDeployments(computeManagementClient, options, function(error, deployments) {
		progress.end();
		return callback(error, deployments);
	});
  },
  ...
  getDockerVMExtension: function (dnsName, options, callback, logger) {
	options.computeManagementClient = this.createComputeManagementClient();
	progress = self.cli.interaction.progress($('Getting docker VM extension.'));
	getVMExtension("MSOpenTech.Extensions.DockerExtension", dnsName, options, logger, function(error, extension) {
		progress.end();
		if(error) {
			callback(error);
		}
		...
		if(extension) {
			...
			logger.warn(util.format($('Extension %s was not found'), "PuppetEnterpriseAgent"));
		}
...
function getVMExtension(extensionName, roleName, options, logger, callback) {
	getDeployments(options.computeManagementClient, options, function (error, deployments) {
		if (error) {
			return callback(error);
		} else {
			var vms = [];
			...
			for (var i = 0; i < extensionsListLength; i++) {
				...
				if(extension.handlerName === extensionName) {
...
function getDeployments(computeManagementClient, options, callback) {
    var deployments = [];
...
function createDockerVM(dnsName, options, logger, callback) {
	...
	checkAndGenerateCertificatesIfNeeded(function(certificateError) {
		...
		createRole(options.dnsPrefix, options.dnsPrefix, options.imageName, options, logger, function(error, role) {
			...
			setDockerVMExtension(role, "3.2", options, logger, function(err, roleWithExtension) {
				logger.data("role");
				logger.json("data", role);
				...
				createVM({
					...
					// We need to wait until dockers extension is installed.
					progress = self.cli.interaction.progress($('Installing docker extension.'));
					return checkIfExtensionIsInstalled();
				}, logger);
			});
		});
	});
	...
	function generateDockerCertificates(cb) {
		...
		fs.writeFile(crlPath, "01", function(err) {
			...
			openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.caKey }, function(err, buffer) {
				if(err) {
					logger.verbose(err); 
				}
				
				openssl.exec('req', { new: true, x509: true, days: 365, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: options.dockerCerts.caKey, out: options.dockerCerts.ca }, function(err, buffer) {
					if(err) {
						logger.verbose(err); 
					}
					
					openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.serverKey }, function(err, buffer) {
						if(err) {
							logger.verbose(err); 
						}
					
						openssl.exec('req', { new: true, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: options.dockerCerts.serverKey, out: options.dockerCerts.server }, function(err, buffer) {
							if(err) {
								logger.verbose(err); 
							}
					
							openssl.exec('x509', { req: true, days: 365, in: options.dockerCerts.server,  passin: 'pass:' + password,
											CA: options.dockerCerts.ca, CAkey: options.dockerCerts.caKey, out: options.dockerCerts.serverCert }, function(err, buffer) {
								if(err) {
										logger.verbose(err.toString()); 
									}
								
								openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.clientKey }, function(err, buffer) {
									if(err) {
										logger.verbose(err); 
									}
					
									openssl.exec('req', { new: true, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: options.dockerCerts.clientKey, out: options.dockerCerts.client }, function(err, buffer) {
										if(err) {
											logger.verbose(err.toString()); 
										}
										...
										fs.writeFile(configPath, "extendedKeyUsage = clientAuth", function(err) {
											...
											openssl.exec('x509', { req: true, days: 365, in: options.dockerCerts.client,  passin: 'pass:' + password, extfile: configPath, CA: options.dockerCerts.ca, CAkey: options.dockerCerts.caKey, out: options.dockerCerts.clientCert }, function(err, buffer) {
												if(err) {
													logger.verbose(err.toString()); 
												}
						
												openssl.exec('rsa', { passin: 'pass:' + password, in: options.dockerCerts.serverKey, passout: 'pass:' + password, out: options.dockerCerts.serverKey }, function(err, buffer) {
													if(err) {
														logger.verbose(err.toString()); 
													}
												
													openssl.exec('rsa', { passin: 'pass:' + password, in: options.dockerCerts.clientKey, passout: 'pass:' + password, out: options.dockerCerts.clientKey }, function(err, buffer) {
														if(err) {
															logger.verbose(err.toString()); 
														}
	...
	function checkIfExtensionIsInstalled(error, extension, previousStatus) {
		var extensionStatus = extension ? extension.status : "Preparing to install";
		if(previousStatus != extensionStatus) {
			logger.info("Extension status: " + extensionStatus);
		}
		if(extension && extension.status !== "Installing") {
			logger.info("Docker extension is installed");
			logger.json("verbose", extension);
			progress.end();
			return callback();
		}
		
		getVMExtension("MSOpenTech.Extensions.DockerExtension", dnsName, options, options.logger, function(error, extension) {
			if(error) {
				return callback(error);
			}
			
			return checkIfExtensionIsInstalled(error, extension, extensionStatus)
		});
	}
}
          ...
          if (error) {
            if (error.statusCode === 404) {
              ...
              var createDeployment_ = function () {
                ...
                computeManagementClient.virtualMachines.createDeployment(dnsPrefix, deploymentParams, logger, function (error) {
			    progress.end();
                  if (!error) {
                    logger.info('OK');
