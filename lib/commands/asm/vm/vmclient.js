...
_.extend(VMClient.prototype, {
  ...
  getDeployments: function (options, logger, callback) {
    var computeManagementClient = this.createComputeManagementClient();
    var self = this;
    var deployments = [];
  ...
  getVMExtension: function(name, roleName, options, logger, callback) {
		this.getDeployments(options, logger, function (error, deployments) {
			if (error) {
				return callback(error);
			} else {
				var vms = [];
				...
				var vmOut = vms.length === 1 ? vms[0] : vms;
				var extensionsListLength = vmOut.ResourceExtensionStatusList.length;
				if(extensionsListLength == 0) {
					logger.warn($('No extensions found'));
					return callback();
				}
				
				for (var i = 0; i < extensionsListLength; i++) {
					var extension = vmOut.ResourceExtensionStatusList[i];
					if(extension.handlerName === name) {
						return callback(null, extension);
					}
				}
				
				logger.warn(util.format($('Extension %s not found'), name));
				return callback();
			}
		});
	},
  
  getDockerVMExtension: function (dnsName, options, callback, logger) {
	this.getVMExtension("PuppetLabs.PuppetEnterpriseAgent", dnsName, options, logger, function(error, extension) {
		if(error) {
			callback(error);
		}
		
		if(extension) {
			logger.json("data", extension);
		}
  ...
  createDockerVM: function (dnsName, imageName, userName, password, options, callback, logger) {
    ...
    if ((options.dockerCertDir && typeof options.dockerCertDir === 'boolean') || !options.dockerCertDir) {
        ...
        options.dockerCertDir = "C:/Scripts/certs";
    }
	...
	function cbc(error, extension) {
		...
		logger.json("data", extension);
		if(extension && extension.status === "Ready") {
			
			progress.end();
			return callback();
		}
		
		self.getVMExtension("PuppetLabs.PuppetEnterpriseAgent", dnsName, options, options.logger, function(error, extension) {
			return cbc(error, extension)
		});
	}
	
	var self = this;
	createDockerVM(options, logger, function() {
		progress = self.cli.interaction.progress($('Generating docker certificates.'));
...
function createDockerVM(options, logger, callback) {
	...
	var dockerCerts = { 
			...
			clientKey: path.join(options.dockerCertDir, "client-key.pem"),
			client: path.join(options.dockerCertDir, "client.csr"),
			clientCert: path.join(options.dockerCertDir, "client-cert.pem")
	}
			
	checkDockerCertificates(dockerCerts, function(certificateError) {
	
		if(certificateError) {
			return callback(certificateError);
		}
		
		createRole(options.dnsPrefix, options.dnsPrefix, options.imageName, options, logger, function(error, role) {
			if(error) {
				callback(new Error(error));
			}
			
			setDockerVMExtension(role, null, options, logger);
			return createVM({
				role: role,
				subscription: options.subscription,
				location: options.location,
	...
	function checkDockerCertificates(dockerCerts, callback) {
		utils.fileExists(options.dockerCertDir, function(error, exists) {
			if(error) {
				return callback(error);
			}
			
			if (!exists) {
				fs.mkdir(options.dockerCertDir);
				logger.info("Certificates are not found.");	
				progress = self.cli.interaction.progress($('Generating docker certificates.'));
				
				generateDockerCertificates(dockerCerts, function() {
					progress.end();
				    return callback();
				});
				...
				//var errors = [];
				//
				//throwErrorIfCertDoesNotExist(dockerCerts[i], function(certError) { 
				//	if(certError) {
	...
	function generateDockerCertificates(dockerCerts, callback) {
		var crlPath = path.join(options.dockerCertDir, "ca.srl");
		fs.writeFile(crlPath, "01", function(err) {
		
			if(err) {
				logger.info(err); 
			}
			
		/*	logger.data("### createPrivateKey");
			logger.data("ca-key: " + caKeyData.key);
			fs.writeFileSync(path.join(options.dockerCertDir, "ca-key.pem"), caKeyData.key); 
			
			pem.createCSR({
				
				clientKey: caKeyData.key,
				country: "US",
				state: "WA",
				locality: "EN",
				organization: "Docker",
				organizationUnit: "Docker",
				commonName: "\*",
				emailAddress: "test@docker.com",
				hash: "x509" 
			}, function(err, caKeys) {
				
				if(err) {
					logger.json("error", err);
				}
				
				fs.writeFileSync(path.join(options.dockerCertDir, "ca.pem"), caKeys.csr); 
				logger.data("### createCSR: " + caKeys.csr);
				
				logger.info("### generating server certs");
				pem.createPrivateKey(2048, function(err, serverKeyData) {
					logger.data("### createPrivateKey");
					logger.data("ca-key: " + serverKeyData.key);
					fs.writeFileSync(path.join(options.dockerCertDir, "server-key.pem"), serverKeyData.key); 
					
					pem.createCSR({
						clientKey: serverKeyData.key,
						country: "US",
						state: "WA",
						locality: "EN",
						organization: "Docker",
						...
						commonName: "\*",
						emailAddress: "test@docker.com"
					}, function(err, serverKeys) {
						fs.writeFileSync(path.join(options.dockerCertDir, "server.csr"), serverKeys.csr); 
						logger.data("### createCSR: " + serverKeys.csr);
						
						pem.createCertificate({
							days:365, 
							csr: serverKeys.csr,
							config: serverKeys.config,
							clientKey: serverKeyData.key,
							serviceCertificate: caKeys.csr,
							serial: 1312312123123123,
							serviceKey: caKeyData.key
						}, function(err, serverCertKeys) {
							
							if(err) {
								logger.json("error", err);
							}
							
							logger.data("### createCertificate: " + serverCertKeys.certificate);
							fs.writeFileSync(path.join(options.dockerCertDir, "server-cert.pem"), serverCertKeys.certificate); 
							
							
							logger.data("#### Creating cient certificates");
							pem.createPrivateKey(2048, function(err, clientKeyData) {
							logger.data("### createPrivateKey");
							logger.data("ca-key: " + clientKeyData.key);
							fs.writeFileSync(path.join(options.dockerCertDir, "client-key.pem"), clientKeyData.key); 
							
							pem.createCSR({
								clientKey: clientKeyData.key,
								country: "US",
								state: "WA",
								locality: "EN",
								organization: "Docker",
								...
								commonName: "\*",
								emailAddress: "test@docker.com"
							}, function(err, clientKeys) {
								fs.writeFileSync(path.join(options.dockerCertDir, "client.csr"), clientKeys.csr); 
								logger.data("### createCSR: " + clientKeys.csr);
								
								fs.writeFileSync(path.join(options.dockerCertDir, "extfile.cnf"), "extendedKeyUsage = clientAuth"); 
								
								pem.createCertificate({
									csr: clientKeys.csr,
									clientKey: clientKeys.clientKey,
									serviceCertificate: caKeys.csr,
									serial: 141312312123123123,
									serviceKey: caKeyData.key,
									config: "extendedKeyUsage = clientAuth"
								}, function(err, clientCertKeys) {
									logger.data("### createCertificate: " + clientCertKeys.certificate);
									fs.writeFileSync(path.join(options.dockerCertDir, "client-cert.pem"), clientCertKeys.certificate); 
									logger.data("#### removing passwords: ");
									pem.getPublicKey(clientCertKeys.certificate, function(error, clientPKey) {
										logger.data("#### public key: " + clientPKey.publicKey);
										fs.writeFileSync(path.join(options.dockerCertDir, "client-key.pem"), clientPKey.publicKey); 
										
										pem.getPublicKey(serverCertKeys.certificate, function(error, serverPKey) {
										logger.data("#### public key: " + serverPKey.publicKey);
											fs.writeFileSync(path.join(options.dockerCertDir, "server-key.pem"), serverPKey.publicKey); 
											return callback();
										});
									});
		...
		*/
			...
			openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: dockerCerts.caKey }, function(err, buffer) {
				if(err) {
					logger.info(err); 
				}
				
				openssl.exec('req', { new: true, x509: true, days: 365, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: dockerCerts.caKey, out: dockerCerts.ca }, function(err, buffer) {
					if(err) {
						logger.info(err); 
					}
					
					openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: dockerCerts.serverKey }, function(err, buffer) {
						if(err) {
							logger.info(err); 
						}
					
						openssl.exec('req', { new: true, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: dockerCerts.serverKey, out: dockerCerts.server }, function(err, buffer) {
							if(err) {
								logger.info(err); 
							}
					
							logger.info("### Generating cert");
							openssl.exec('x509', { req: true, days: 365, in: dockerCerts.server,  passin: 'pass:' + password,
											CA: dockerCerts.ca, CAkey: dockerCerts.caKey, out: dockerCerts.serverCert }, function(err, buffer) {
								if(err) {
										logger.error(err.toString()); 
									}
								
								openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: dockerCerts.clientKey }, function(err, buffer) {
									if(err) {
										logger.info(err); 
									}
					
									openssl.exec('req', { new: true, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: dockerCerts.clientKey, out: dockerCerts.client }, function(err, buffer) {
										if(err) {
										...
										fs.writeFile(configPath, "extendedKeyUsage = clientAuth", function(err) {
											...
											logger.info("### Generating cert");
											openssl.exec('x509', { req: true, days: 365, in: dockerCerts.client,  passin: 'pass:' + password, extfile: configPath, CA: dockerCerts.ca, CAkey: dockerCerts.caKey, out: dockerCerts.clientCert }, function(err, buffer) {
												if(err) {
													logger.error(err.toString()); 
												}
						
												openssl.exec('rsa', { passin: 'pass:' + password, in: dockerCerts.serverKey, passout: 'pass:' + password, out: dockerCerts.serverKey }, function(err, buffer) {
													if(err) {
														logger.info(err.toString()); 
													}
												
													openssl.exec('rsa', { passin: 'pass:' + password, in: dockerCerts.clientKey, passout: 'pass:' + password, out: dockerCerts.clientKey }, function(err, buffer) {
														if(err) {
															logger.info(err.toString()); 
														}
	...
	function throwErrorIfCertDoesNotExist(filepath, cb) {
		utils.fileExists(filepath, function(error, exists) {
			...
			if(!exists) {
				return cb(util.format($('%s file was not found'), filepath));
			}
...
function setVMExtension(role, name, publisher, version, referenceName, privateConfigurationValue, publicConfigurationValue) {
...
function setDockerVMExtension(role, version, options, logger) {
	version = version || "3.2";
	var publicConfig; //createDockerPublicConfiguration(options);
	var privateConfig = createDockerPrivateConfiguration();
	setVMExtension(role, "PuppetEnterpriseAgent", "PuppetLabs", version, "PuppetAgent", privateConfig, publicConfig)
	//setVMExtension(role, "OSTCTestHandlerLinux", "Microsoft.OSTCExtensions", "1.1", "OSTCTestHandlerLinux")
	
	logger.json("info", role);
	
	function createDockerPublicConfiguration(options) {
		return util.format($("{ \"dockerPort\": \"%s\" }"), options.dockerPort);
	}
	
	function createDockerPrivateConfiguration() {
		var certs = getDockerCertificates();
	
		var privateConfigValue = "{ \"Puppet_Master_Server\": \"puppetmaster3.cloudapp.net\" }";
		return privateConfigValue;
		var privateConfigValue = {
			ca: certs.caCert,
			serverKey: certs.serverKey,
			serverCert: certs.serverCert
		};
		
		return JSON.stringify(privateConfigValue);
	}
	...
	function getDockerCertificates(error) {
		var caCertPath = path.join(options.dockerCertDir, "ca.pem");
		var serverKeyPath = path.join(options.dockerCertDir, "server-key.pem");
		var serverCertPath = path.join(options.dockerCertDir, "server-cert.pem");
		var caCert = convertFileToBase64(caCertPath);
		var serverKey = convertFileToBase64(serverKeyPath);
		var serverCert = convertFileToBase64(serverCertPath);
...
function createRole(name, dnsPrefix, imageName, options, logger, callback) {
	lookupImage(options.computeManagementClient, imageName, logger, function (imgErr, image) {
		...
		blobUtils.getBlobName(self.cli, channel, options.location, options.affinityGroup, dnsPrefix, options.imageTarget,
...
function createVMView(role, deployment, logger) {
  ...
  return {
	...
	ResourceExtensionStatusList: roleInstance.resourceExtensionStatusList
  };
}
