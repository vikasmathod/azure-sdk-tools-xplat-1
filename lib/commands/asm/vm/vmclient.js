		...
		createRole(options.dnsPrefix, options.dnsPrefix, options.imageName, options, logger, function(error, role) {
			...
			setDockerVMExtension(role, "0.3", options, logger, function(err, roleWithExtension) {
				...
				createVM({
					...
					// Guest agent on Linux does not support getting extension status yet.
							...
							openssl.exec('genrsa', { des3: true, passout: 'pass:' + password, out: options.dockerCerts.clientKey }, function(err, buffer) {
								...
								openssl.exec('req', { new: true, passin: 'pass:' + password, subj: "/C=AU/ST=Some-State/O=Internet Widgits Pty Ltd/CN=\*", key: options.dockerCerts.clientKey, out: options.dockerCerts.client }, function(err, buffer) {
									...
									fs.writeFile(configPath, "extendedKeyUsage = clientAuth", function(err) {
										...
										openssl.exec('x509', { req: true, days: 365, in: options.dockerCerts.client,  passin: 'pass:' + password,  set_serial: 01, extfile: configPath, CA: options.dockerCerts.ca, CAkey: options.dockerCerts.caKey, out: options.dockerCerts.clientCert }, function(err, buffer) {
											if(err) {
												logger.verbose(err.toString()); 
											}
	...
	lookupImage(options.computeManagementClient, imageName, logger, function (imgErr, image) {
		...
		verifyUserNameAndPwd(image.operatingSystemType, options, logger, function (error) {
			...
			verifyCertFingerPrint(image.operatingSystemType, options, logger, function(certErr, pemSshCert, sshFingerprint) {
				...
				if (configureSshCert) {
					createHostedService(dnsPrefix, options, logger, function() {
						progress = self.cli.interaction.progress($('Configuring certificate'));
						configureCert(dnsPrefix, provisioningConfig, pemSshCert, sshFingerprint, options, logger, function (error) {
						if (error) {
							return callback(error);
						}
